...
##Additional package for AUROC plotting
...
def train(model,
    ...
    # Evaluate the model
    accuracy, f1, precision, recall, auroc, auprc = evaluate(model, criterion, test_loader, device, plot_roc=True, output_dir=output_dir)
...
#############################
# Updated eval function of AUROC plotting
#############################
def evaluate(model, criterion, val_loader, device, plot_roc=False, output_dir=None):
    """
    Evaluate the linear probe model.
    ...
    # Ensure that pred_gather and target_gather have consistent lengths
    assert pred_gather.shape[0] == target_gather.shape[0], f"Predictions and targets must have the same number of samples. Got {pred_gather.shape[0]} and {target_gather.shape[0]}."
    ...
    if plot_roc and output_dir:
        plot_roc_curve(target_gather, pred_gather, output_dir)
